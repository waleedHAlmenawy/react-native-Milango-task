...
const RepoScreen = ({isDarkMode}: {isDarkMode: boolean}) => {
  ...
  useEffect(() => {
    const temp: any = [
      ...new Set(
        repositories.map((item: {language: string}) => {
          if (item.language) return item.language;
        }),
      ),
    ].filter(lang => lang);

    setProgrammingLanguages(temp);
    setLang('');
    setRepos([...repositories]);
  }, [repositories]);

  useEffect(() => {
    setRepos(repositories.filter((repo: any) => repo.language === lang));
  }, [lang]);
            ...
            <View
              ...
              <View>
                <TextInput
                  ...
                  onChangeText={text => {
                    setLangs(() =>
                      programmingLanguages.filter((item: string) =>
              ...
              <View>
                <DatePicker
                  ...
                  options={{
                    backgroundColor: (isDarkMode ? Colors.dark : Colors.light)
                      .background,
                    textHeaderColor: (isDarkMode ? Colors.dark : Colors.light)
                      .text,
                    textDefaultColor: (isDarkMode ? Colors.dark : Colors.light)
                      .text,
                    selectedTextColor: (isDarkMode ? Colors.dark : Colors.light)
                      .text,
                    mainColor: '#68DDBA',
